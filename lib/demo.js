...
// main function
  ...
  async getrequiredIssues () {
    ...
    if (ExcludeMilestones.length!=0){
      for (var i in ExcludeMilestones){
      q+='-milestone:'+ExcludeMilestones[i]+' '
    }
    }
...
async issuelocate(q){
  ...
  const params = {q, sort: 'updated', order: 'desc', per_page: 100}
  // this params is a new built dictionary with : q,sort,order,per_page
  const issues = await this.github.search.issues(params)
